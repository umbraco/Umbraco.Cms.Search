...
The Umbraco Search client publishes TypeScript types as a types-only npm package, `@umbraco-cms/search`. Install it as a dev dependency so you can author your extension with full IntelliSense for contexts, manifests, and shared models:
...
- `@umbraco-cms/search/global` - constants and global contexts loaded upfront (entity types, repository aliases, notification context)
- `@umbraco-cms/search/settings` - the lazy-loaded implementation surface (workspace context tokens, repositories, models)

No runtime dependency is shipped - the actual JavaScript is served by the Umbraco backoffice itself at runtime via an importmap. You only need the types at build time:
...
The package declares `@umbraco-cms/backoffice` as a peer dependency, so make sure that is installed in your project too.

## Adding a detail box to the search workspace

<figure>
    <img alt="Search index detail boxes" src="images/searchIndexDetailBox.jpg" />
    <figcaption>The marked areas illustrate locations of search index detail boxes</figcaption>
</figure>
...
- `label` - The box heading. Supports localization keys (e.g. `#myPackage_myLabel`).
...
Higher `weight` values render first within a column.
...
| `search-index` | `UMB_SEARCH_INDEX_ENTITY_TYPE` | Per-index actions (e.g. Rebuild Index) |
...
For provider developers who need deep-linkable modals (e.g. viewing detailed document information), Umbraco Search supports a routable modal pattern. This uses a non-visual `searchIndexDetailBox` as a route registration host, combined with an entity action that provides navigable URLs.
...
- `addAdditionalPath()` takes a **single string** with all parameters combined (e.g. `':documentUnique/:culture'`). Calling it twice overwrites the first value.
...
To import from `@umbraco-cms/search/settings` (e.g. `UMB_SEARCH_WORKSPACE_CONTEXT`) in your provider package, add path mappings in your `tsconfig.json`:
